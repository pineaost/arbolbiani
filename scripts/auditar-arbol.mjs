...
const lineas = trazos.map(({trazo})=>trazo?(trazo.partes??[{d:trazo.d,papel:"descendencia"}]).map(p=>`<path d="${p.d}" fill="none" stroke="${p.papel==="union"?"#49355f":"#836d6e"}" opacity="${p.papel==="hermanos"?0.48:0.85}" stroke-width="${p.papel==="union"?2:1.6}"/>`).join(""):"").join("");
